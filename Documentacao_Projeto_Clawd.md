...
O visual padrão é o sprite compacto vermelho de referência. Em repouso, as pernas permanecem imóveis; elas só alternam durante caminhada, corrida, inércia e embaixadinhas. Piscadas, bocas e balões de emoji são camadas independentes, portanto não deformam o corpo base. Os oito sub-pets usam sprites pixel maiores (grade ~12×9) definidos em `CLAWD_SUBPET_SPRITES`.
...
- **Sub-pets:** oito espécies com arte distinta, apelido, cores independentes de corpo e olhos, sono, despertar sincronizado, seis ações manuais e habilidades especiais;
...
A suíte local usa `node:test` e verificações de sintaxe para validar estado, migrações, catálogo, acessórios, sprite, emoções, modo liso, sub-pets, documentação, pesca, manifest, popup, reload seguro, bfcache, áudio pós-gesto e allowlists. Em 15/07/2026, **61/61 contratos** passaram; o smoke test em Chromium confirmou 14 acessórios em cada renderizador, provador pixel/liso, equipamentos profissionais temporários, 8 profissões, ações do catálogo (24), pesca cancelada/concluída, 7 ações do subpet, sprites unificados em `CLAWD_SUBPET_SPRITES`, cor independente dos olhos, três reloads com uma única instância e zero erros de runtime.
