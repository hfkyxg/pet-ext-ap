...
- **Renderização adaptativa**: deslocamento e sub-pet usam `requestAnimationFrame`; o content script mede a cadência real em 30 frames e registra `data-refresh-rate`. O follow do subpet multiplica o spring por `dt` (normalizado a 60 fps, cap 3×) para manter a mesma sensação em monitores 60/120/144 Hz; fora da viewport, `IntersectionObserver` pausa rAF e CSS (`.off-screen`);
...
| **SubPet RAF guard** | `_loop()` do SubPet verifica `document.body.contains(this.node)` antes de continuar; termina silenciosamente ao detectar nó removido. Follow spring usa `dt` (60 fps); `IntersectionObserver` pausa rAF/CSS off-screen. |

### v3.7 — Animações, Partículas e Interações

| Feature | Detalhe técnico |
|---------|----------------|
| **Hover pixel-art** | Hover no chão (`.ground-shadow`) — sem `filter` no `.pet-body` scaleado (evita AA pastoso no modo liso). |
