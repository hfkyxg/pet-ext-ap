...
A suíte local usa `node:test` e verificações de sintaxe para validar estado, migrações, catálogo, acessórios, sprite, emoções, modo liso, sub-pets, documentação, pesca, manifest, popup, reload seguro, bfcache, áudio pós-gesto, allowlists, save coalesce, gamificação e harmonia da fase (contexto/weekly/personalidade). Em 17/07/2026, **153/153 contratos** passaram; o catálogo vivo registra **30** ações no popup (+ kick/keepy/superdance), **31** acessórios, **12** profissões, **34** conquistas, **14** quests e **12** desafios semanais; a bola do Jogador fica no pé direito (pixel sem blur); o smoke test em Chromium confirma sprites unificados em `CLAWD_SUBPET_SPRITES` (+ PNGs), três reloads com uma única instância e zero erros de runtime.
