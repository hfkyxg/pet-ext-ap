(() => {
  ...
  function setupMainPetLab() {
    ...
    // Aplica cor/olhos iniciais (pêssego Claude no lab)
    pet.style.setProperty('--agent-color', bodyColor.value);
    pixelArt.style.setProperty('--agent-color', bodyColor.value);
    pet.style.setProperty('--agent-eye-color', eyeColor.value);
    pixelArt.style.setProperty('--agent-eye-color', eyeColor.value);
  ...
  function paintSubpet(species, pose = 'idle', frameIndex = 0) {
  ...
  function setupSubpetLab() {
    ...
    $('subpet-body-color').addEventListener('input', event => {
      ...
      paintSubpet(select.value, 'auto', Number(preview.dataset.frame || 0));
    });
    ...
    $('subpet-nickname').addEventListener('input', () => paintSubpet(select.value, 'auto', Number(preview.dataset.frame || 0)));
    actionGrid.addEventListener('click', event => {
      ...
      const sprite = subpetCatalog[select.value];
      if (action === 'special' && sprite?.frames?.special) paintSubpet(select.value, 'special', 0);
      else if (sprite?.frames?.flying && (select.value === 'dragon' || select.value === 'bird')) paintSubpet(select.value, 'flying', 0);
      else if (sprite?.frames?.walk) paintSubpet(select.value, 'walk', 0);
      bubble.textContent = action === 'special'
        ? `${nickname}: ${subpetSpecialSpeech[select.value]}`
        : `${item.emoji} ${nickname}: ${item.feedback}`;
      actionTimer = setTimeout(() => {
        preview.className = 'subpet-preview';
        paintSubpet(select.value, 'auto', 0);
