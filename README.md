...
Não há *build step* nem dependências externas: você clona, carrega a pasta no `chrome://extensions` e pronto. Por baixo, um **service worker** MV3 cuida da inicialização, o conteúdo se auto-recupera de recargas da extensão, e **51 testes automatizados** (mais um *smoke test* em Chromium real) guardam catálogos, schema, renderização, a mecânica das embaixadinhas e a integridade cruzada de todos os subsistemas. É um brinquedo — mas construído como software de verdade.
